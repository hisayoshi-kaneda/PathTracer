...
public:
    ...
    void set_uniform_value(glm::ivec3 val, const char *val_name) {
        GLuint loc_id = glGetUniformLocation(program_id, val_name);
        glUniform3iv(loc_id, 1, glm::value_ptr(val));
    }

    void set_uniform_value(glm::vec3 *val, int size, const char *val_name) {
        GLuint loc_id = glGetUniformLocation(program_id, val_name);
        glUniform3fv(loc_id, size, glm::value_ptr(val[0]));
    }
